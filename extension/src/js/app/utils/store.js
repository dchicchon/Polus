    ...
    if (state.signedIn) {
      ...
      if (update) {
        if (Object.keys(dateObject).length > 0) {
          ...
          const q = query(collection(db, 'users', state.uid, date), where('__name__', 'not-in', Object.keys(dateObject)))
          ...
          const querySnapshot = await getDocs(collection(db, 'users', state.uid, date))
          if (querySnapshot.size !== 0) {
            querySnapshot.forEach((doc) => {
